...
export async function GET(req: Request) {
  ...
  const seed = (searchParams.get('seed') || 'a').trim() || 'a'
  ...
  try {
    // Deezer chart endpoint can be very limited (often no `next`, small `total`).
    // For a reliable infinite-scroll "discover" list, use search with a seed query + index/limit.
    const url = `https://api.deezer.com/search?q=${encodeURIComponent(seed)}&index=${index}&limit=${limit}`
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        { error: 'Deezer trending failed', detail: txt.slice(0, 2000) },
        { status: 502 }
      )
    }
    ...
    return NextResponse.json({ data, nextIndex, seed, total })
  } catch (e) {
    console.error('Deezer trending error:', e)
    return NextResponse.json({ error: 'Deezer trending error' }, { status: 500 })
  }
}
